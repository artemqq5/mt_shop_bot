...
from bot.domain.states.admin.management.AddItemState import AddItemState
...
from bot.presentation.keyboards.admin.management.category.kb_managment import (
    CategoryManagementAddItem,
    kb_back_category_management,
)
from bot.presentation.keyboards.admin.management.item.kb_add_item import (
    PreviewItemPublish,
    kb_preview_add_item,
    kb_publish_onemore,
)
...
async def add_item_management(callback: CallbackQuery, state: FSMContext, i18n: I18nContext):
    ...
    await callback.message.edit_text(
        i18n.ADMIN.TITLE_ITEM(), reply_markup=kb_back_category_management(data["category"])
    )
...
async def set_cost_item(message: types.Message, state: FSMContext, i18n: I18nContext):
    ...
    await message.answer(
        i18n.ADMIN.PREVIEW_ITEM(title=data["title"], category=data["category"], cost=data["cost"], desc=data["desc"]),
...
async def preview(callback: CallbackQuery, state: FSMContext, i18n: I18nContext):
    ...
    if mode == "publish":
        if ItemRepository().add(data["title"], data["desc"], data["category"], data["cost"]):
            await callback.message.edit_text(
                i18n.ADMIN.SUCCESS_PUBLISHED(), reply_markup=kb_publish_onemore(data["category"])
            )
        else:
            await callback.message.edit_text(
                i18n.ADMIN.FAIL_PUBLISHED(), reply_markup=kb_back_category_management(data["category"])
            )
    elif mode == "restart":
