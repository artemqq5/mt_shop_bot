...
def kb_profile_orders(orders, current_page=1):
    ...
    for i in range(start_index, end_index):
        ...
        inline_kb.append(
            [InlineKeyboardButton(
                text=L.CLIENT.PROFILE.ORDER_LIST_TEMPLATE(title=item['title'], count=orders[i]['count'],
    ...
    if current_page > 1:
        nav.append(InlineKeyboardButton(
            text='<',
            callback_data=ProfileOrdersNav(page=current_page - 1).pack()
        ))
    else:
        nav.append(InlineKeyboardButton(
            text='<',
    ...
    inline_kb.append([
        InlineKeyboardButton(text=L.BACK(), callback_data=ProfileBack().pack())
    ])
...
kb_profile_orders_back = InlineKeyboardMarkup(inline_keyboard=[
    [InlineKeyboardButton(text=L.BACK(), callback_data=ProfileOrdersBack().pack())]
])
