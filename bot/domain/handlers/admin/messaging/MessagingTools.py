    ...
    async def preview_message_send(data, bot, user_id):
        ...
        if data.get("photo", None):
            await bot.send_photo(
                chat_id=user_id, photo=data.get("photo"), caption=data["message"], reply_markup=kb_buttons
            )
        elif data.get("video", None):
            await bot.send_video(
                chat_id=user_id, video=data.get("video"), caption=data["message"], reply_markup=kb_buttons
            )
        elif data.get("animation", None):
            await bot.send_animation(
                chat_id=user_id, animation=data.get("animation"), caption=data["message"], reply_markup=kb_buttons
            )
        else:
            await bot.send_message(chat_id=user_id, text=data["message"], reply_markup=kb_buttons)
