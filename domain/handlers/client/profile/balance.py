...
from data.whitepay import WhitePayRepository
from domain.notification.NotificationAdmin import NotificationAdmin
from domain.states.client.profile.BalanceState import BalanceState
from presentation.keyboards.client.profile.kb_balance import BalanceReplenish, kb_pay_invoice
...
async def balance_pay_invoice(message: types.Message, state: FSMContext, i18n: I18nContext, bot: Bot):
    ...
    await message.answer(i18n.CLIENT.BALANCE_INFO(
        id=invoice_response['order']['id'], sum=invoice_response['order']['value'],
