...
from presentation.keyboards.admin.kb_managment import CategoryManagementItemList, ManagementBack
    ...
    for i in range(start_index, end_index):
        inline_kb.append(
            [InlineKeyboardButton(
                text=items[i]['title'],
                callback_data=ItemChoice(id=items[i]['id']).pack()
            )]
        )
...
def kb_back_choice_item(item):
    ...
    if item['visibility'] == 1:
        inline_kb.append([InlineKeyboardButton(
            text=L.ADMIN.HIDE(),
            callback_data=ItemManagementVisibility(visibility=False, id=item['id']).pack()
        )])
    else:
        inline_kb.append([InlineKeyboardButton(
            text=L.ADMIN.OPEN(),
            callback_data=ItemManagementVisibility(visibility=True, id=item['id']).pack()
        )])
