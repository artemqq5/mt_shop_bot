...
def kb_ban_list_nav(ban_users, current_page: int = 1):
    ...
    # Navigation buttons
    if current_page > 1:
        nav.append(InlineKeyboardButton(text="<", callback_data=BanListNavigation(page=current_page - 1).pack()))
    else:
...
def text_ban_list(ban_users, i18n, current_page: int = 1):
    ...
    for i in range(start_index, end_index):
        message += i18n.ADMIN.BAN_LIST_TEMPLATE(
            username=ban_users[i]["username"], id=ban_users[i]["user_id"], lang=str(ban_users[i]["lang"])
        )
        message += "\n\n"
