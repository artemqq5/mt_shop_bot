...
async def choice_buy_item(callback: CallbackQuery, state: FSMContext, i18n: I18nContext):
    ...
    await callback.message.edit_text(
        i18n.CLIENT.BUY.ITEM_TEMPLATE(
            title=item["title"], category=item["category"], cost=item["cost"], desc=item["desc"]
        ),
...
async def item_buy_back(callback: CallbackQuery, state: FSMContext, i18n: I18nContext):
    ...
    await callback.message.edit_text(
        text=i18n.CLIENT.BUY.CHOICE_ITEM(category=data["category"]), reply_markup=kb_buy_item_choice(items, page)
    )
