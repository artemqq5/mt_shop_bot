...
from bot.domain.handlers.admin.management.category import (
    add_category,
    delete_category,
    visibility_category,
)
from bot.domain.handlers.admin.management.item import choice_item
...
from bot.presentation.keyboards.admin.management.category.kb_managment import (
    ...
    ChoiceCategoryBack,
    ManagementBack,
    kb_category_management,
    kb_choice_category,
)

router = Router()
router.include_routers(add_category.router, delete_category.router, visibility_category.router, choice_item.router)
...
async def choice_category_back(callback: CallbackQuery, state: FSMContext, i18n: I18nContext):
    ...
    await callback.message.edit_text(i18n.ADMIN.CHOICE_CATEGORY(), reply_markup=kb_choice_category(categories, page))
