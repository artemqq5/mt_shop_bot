...
def kb_buy_item_choice(items, current_page=1):
    ...
    # Navigation buttons
    if current_page > 1:
        nav.append(InlineKeyboardButton(text="<", callback_data=BuyItemNavigation(page=current_page - 1).pack()))
    else:
    ...
    inline_kb.append([InlineKeyboardButton(text=L.BACK(), callback_data=BuyChoiceCategoryBack().pack())])
