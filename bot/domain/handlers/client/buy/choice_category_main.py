...
from bot.domain.handlers.client.buy import buy_item, choice_item
from bot.domain.states.client.buy.BuyItemState import BuyItemState
from bot.presentation.keyboards.client.buy.kb_buy_category import (
    BuyCategoryChoice,
    BuyCategoryNavigation,
    kb_buy_category_choice,
)
from bot.presentation.keyboards.client.buy.kb_buy_item import (
    BuyChoiceCategoryBack,
    kb_buy_item_choice,
)

router = Router()

router.include_routers(choice_item.router, buy_item.router)
...
async def choice_buy_category(callback: CallbackQuery, state: FSMContext, i18n: I18nContext):
    ...
    await callback.message.edit_text(
        text=i18n.CLIENT.BUY.CHOICE_ITEM(category=category), reply_markup=kb_buy_item_choice(items)
    )
...
async def choice_buy_category_back(callback: CallbackQuery, state: FSMContext, i18n: I18nContext):
    ...
    await callback.message.edit_text(
        i18n.CLIENT.BUY.CHOICE_CATEGORY(), reply_markup=kb_buy_category_choice(categories, page)
    )
