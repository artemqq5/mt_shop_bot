...
from bot.presentation.keyboards.admin.ban.kb_ban_list import (
    BanListNavigation,
    kb_ban_list_nav,
    text_ban_list,
)
from bot.presentation.keyboards.admin.ban.kb_ban_system import ListBannedUsersCallback
...
async def ban_list_user_callback(callback: CallbackQuery, state: FSMContext, i18n: I18nContext):
    ...
    await callback.message.edit_text(text_ban_list(ban_users, i18n), reply_markup=kb_ban_list_nav(ban_users))
...
async def ban_list_navigation(callback: CallbackQuery, state: FSMContext, i18n: I18nContext):
    ...
    await callback.message.edit_text(
        text_ban_list(ban_users, i18n, int(page)), reply_markup=kb_ban_list_nav(ban_users, int(page))
    )
