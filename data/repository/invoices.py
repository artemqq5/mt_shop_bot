...
class InvoiceRepository(MyDataBase):
    ...
    def add(self, external_id, number, expected_amount, user_id, username, firstname, date):
        query = "INSERT INTO `invoices` (`external_id`, `number`, `expected_amount`, `user_id`, `username`, `firstname`, `date`) VALUE (%s, %s, %s, %s, %s, %s, %s);"
        return self._insert(query, (external_id, number, expected_amount, user_id, username, firstname, date))
