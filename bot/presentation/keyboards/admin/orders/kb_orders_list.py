...
def kb_orders_choice(orders, current_page=1):
    ...
    # Navigation buttons
    if current_page > 1:
        nav.append(InlineKeyboardButton(text="<", callback_data=OrderItemNavigation(page=current_page - 1).pack()))
    else:
...
def kb_order_back(page):
    return InlineKeyboardMarkup(
        inline_keyboard=[[InlineKeyboardButton(text=L.BACK(), callback_data=OrderItemBack(page=page).pack())]]
    )
