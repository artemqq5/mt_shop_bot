...
from bot.presentation.keyboards.admin.management.category.kb_delete_category import (
    CategoryApproveDelete,
    kb_category_delete,
)
...
async def delete_category(callback: CallbackQuery, state: FSMContext, i18n: I18nContext):
    ...
    await callback.message.edit_text(
        i18n.ADMIN.DELETE_CATEGORY_APPROVE(category=data["category"]), reply_markup=kb_category_delete(data["category"])
    )
...
async def delete_approve_category(callback: CallbackQuery, state: FSMContext, i18n: I18nContext):
    ...
    if CategoryRepository().delete(data["category"]):
        ...
        await callback.message.edit_text(i18n.ADMIN.DELETE_SUCCESS(), reply_markup=kb_back_category_choice)
    else:
        await callback.message.edit_text(i18n.ADMIN.DELETE_FAIL(), reply_markup=kb_back_category_choice)
