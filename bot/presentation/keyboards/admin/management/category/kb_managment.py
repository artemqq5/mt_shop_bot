...
def kb_choice_category(categories, current_page: int = 1):
    # create new category
    inline_kb = [[InlineKeyboardButton(text=L.ADMIN.CREATE_NEW_CATEGORY(), callback_data=CreateNewCategory().pack())]]
    ...
    # Navigation buttons
    if current_page > 1:
        nav.append(InlineKeyboardButton(text="<", callback_data=CategoryNavigation(page=current_page - 1).pack()))
    else:
...
def kb_category_management(category):
    inline_kb = [
        [InlineKeyboardButton(text=L.ADMIN.ADD_ITEM(), callback_data=CategoryManagementAddItem().pack())],
        [InlineKeyboardButton(text=L.ADMIN.SHOW_ITEMS(), callback_data=CategoryManagementItemList().pack())],
    ]
    ...
    if category["visibility"] == 1:
        inline_kb.append(
            [
                InlineKeyboardButton(
                    text=L.ADMIN.HIDE(), callback_data=CategoryManagementVisibility(visibility=False).pack()
                )
            ]
        )
    else:
        inline_kb.append(
            [
                InlineKeyboardButton(
                    text=L.ADMIN.OPEN(), callback_data=CategoryManagementVisibility(visibility=True).pack()
                )
            ]
        )

    inline_kb.append([InlineKeyboardButton(text=L.ADMIN.DELETE(), callback_data=CategoryManagementDelete().pack())])

    inline_kb.append([InlineKeyboardButton(text=L.BACK(), callback_data=ChoiceCategoryBack().pack())])
...
def kb_back_category_management(category):
    return InlineKeyboardMarkup(
        inline_keyboard=[[InlineKeyboardButton(text=L.BACK(), callback_data=ManagementBack(category=category).pack())]]
    )


kb_back_category_choice = InlineKeyboardMarkup(
    inline_keyboard=[[InlineKeyboardButton(text=L.BACK(), callback_data=ChoiceCategoryBack().pack())]]
)
